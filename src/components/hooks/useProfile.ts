...
export function useProfile(): UseProfileResult {
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      try {
        ...
        setProfile(null);
        setStatus("error");
        setErrorMessage(err instanceof ApiError ? "Nie udało się pobrać profilu" : "Nieoczekiwany błąd");
      }
    }
  ...
  return useMemo(() => ({ status, profile, errorMessage, refetch }), [status, profile, errorMessage, refetch]);
}
