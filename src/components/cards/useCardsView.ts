...
function safeOneOf<T extends string>(
  value: string | null,
  allowed: readonly T[]
): T | null {
  if (!value) return null
...
function queryFromUrl(): CardsQueryState {
  ...
  const sort =
    safeOneOf(params.get("sort"), ["created_at", "updated_at", "front"] as const) ??
    DEFAULT_QUERY.sort
  const order =
    safeOneOf(params.get("order"), ["asc", "desc"] as const) ?? DEFAULT_QUERY.order

  const source =
    safeOneOf(params.get("source"), ["manual", "ai_created", "ai_edited"] as const) ??
    undefined
  ...
  const actions: CardsViewActions = useMemo(
    () => ({
      ...
      setGenerationId: (value) =>
        setQuery((q) => ({ ...q, generation_id: value, page: 1 })),
      setPage: (page) =>
        setQuery((q) => ({ ...q, page: Math.max(1, Math.floor(page)) })),
      setLimit: (limit) =>
        setQuery((q) => ({
          ...q,
      ...
      createCard: async (draft: CreateCardDraftVM) => {
        ...
        try {
          ...
          toast.success(
            `Dodano ${res.inserted} ${res.inserted === 1 ? "fiszkę" : "fiszek"}`,
      ...
      confirmDelete: async () => {
        ...
        setDeleteState((s) =>
          s ? { ...s, isSubmitting: true, error: null } : s
        )
