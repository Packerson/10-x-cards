...
export const GET: APIRoute = async ({ params, locals }) => {
  ...
  if (!supabase) {
    return new Response(JSON.stringify({ error: "server_error", details: "supabase_not_configured" }), {
      status: 500,
...
export const PATCH: APIRoute = async ({ params, request, locals }) => {
  ...
  if (!bodyValidation.success) {
    return new Response(
      JSON.stringify({ error: "validation_error", details: bodyValidation.error.format() }),
      { status: 400, headers: { "Content-Type": "application/json" } },
    )
  }
