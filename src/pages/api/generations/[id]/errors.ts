...
export const GET: APIRoute = async ({ params, url, locals }) => {
  ...
  if (!queryValidation.success) {
    return new Response(JSON.stringify({ error: "validation_error", details: queryValidation.error.format() }), {
      status: 400,
      headers: { "Content-Type": "application/json" },
    });
  }
  ...
  const { data, error } = await listGenerationErrors({ supabase, userId }, generationId, queryValidation.data);
