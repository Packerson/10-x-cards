...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  const missingGenerationId = cards.some(
    (card) => (card.source === "ai_created" || card.source === "ai_edited") && card.generation_id === undefined
  );
  ...
  const generationIds = Array.from(
    new Set(cards.map((card) => card.generation_id).filter((id): id is number => typeof id === "number"))
  );

  const supabase = locals.supabase ?? supabaseClient;
  const userId = locals.userId;

  let generationsForRequest: { id: number; total_generated: number | null }[] | null = null;
  ...
  if (generationIds.length > 0) {
    ...
    if (generationError) {
      return new Response(JSON.stringify({ error: "server_error", details: generationError.message }), {
        status: 500,
        headers: { "Content-Type": "application/json" },
      });
    }
  ...
  if (generationIds.length > 0) {
    ...
    for (const card of cards) {
      ...
      acceptedByGenerationId.set(card.generation_id, (acceptedByGenerationId.get(card.generation_id) ?? 0) + 1);
    }
