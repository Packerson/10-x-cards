...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  try {
    ...
    return new Response(JSON.stringify({ error: "validation_error", details: "invalid_json" }), { status: 400 });
  }

  const userId = locals.userId;
  const supabase = locals.supabase ?? supabaseClient;

  const { data, error } = await createGeneration({ supabase, userId }, payload as CreateGenerationCommand);

  if (error) {
    if (error.code === "validation_error") {
      return new Response(JSON.stringify({ error: "validation_error", details: error.details }), { status: 400 });
    }
    ...
    if (error.code === "config_error") {
      return new Response(JSON.stringify({ error: "server_error", details: "openrouter_not_configured" }), {
        status: 500,
      });
    }
    ...
    return new Response(JSON.stringify({ error: "server_error", details: error.details }), { status: 500 });
  }
