...
export async function createCards(
  ...
  try {
    ...
    const { error, count } = await supabase.from("cards").insert(rows, { count: "exact" });
...
type GetCardByIdError = { code: "not_found" } | { code: "database_error"; details?: unknown };
...
interface GetCardForUpdateMeta {
  id: number;
  source: string;
}

type GetCardForUpdateError = { code: "not_found" } | { code: "database_error"; details?: unknown };
...
type DeleteCardError = { code: "not_found" } | { code: "database_error"; details?: unknown };
