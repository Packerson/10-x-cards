...
export class OpenRouterService {
  ...
  public async createChatCompletion(input: ChatCompletionInput): Promise<OpenRouterCompletionDTO> {
  ...
  public async createStructuredCompletion<T = unknown>(input: StructuredCompletionInput): Promise<T> {
  ...
  private async _request(payload: Record<string, unknown>): Promise<OpenRouterRawResponse> {
    ...
    while (attempt <= maxRetries) {
      ...
      try {
        ...
        throw new OpenRouterRequestError("OpenRouter zwrócił błąd odpowiedzi.", response.status, details);
      } catch (error) {
  ...
  private _parseStructuredOutput<T>(raw: OpenRouterRawResponse, responseFormat: OpenRouterResponseFormat): T {
  ...
  private _validateStructuredOutput(data: unknown, schema: Record<string, unknown>) {
    ...
    if (additionalProperties === false && properties && typeof data === "object" && data && !Array.isArray(data)) {
